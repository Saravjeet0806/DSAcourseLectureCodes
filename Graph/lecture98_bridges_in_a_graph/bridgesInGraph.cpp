...
private:
    void dfs(int node, int parent, int &timer, vector<int> &disc,
        ...
        visited[node] = true;
        disc[node] = low[node] = timer++;   //disc[node] = timer , low[node] = timer and timer++;
        ...
        for(auto neigh : adj[node]) {                       
            ...
            if(visited[neigh] == false) {
                dfs(neigh, node, timer, disc, low, result, adj, visited);
                low[node] = min(low[node], low[neigh]);
...
public:
    vector<vector<int>> criticalConnections(int n, vector<vector<int>>& connections) {
        ...
        int timer = 0;
        vector<int> disc(n, -1);  //discovery_time
        vector<int> low(n, -1);   //lowest_reachable_time
        int parent = -1;
        vector<bool> visited(n, false);
        vector<vector<int>> result;
